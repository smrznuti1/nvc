local function exit_zen_if_active()
  ...
  if
    (zen_filetype == 'floaterm' and vim.fn.bufnr() == zen.win.buf)
...
function CompletionForRun(arg_lead, _, _)
  ...
  file_completions = vim.tbl_map(
    function(item) return item:gsub('([ ()%%#$])', '\\%1') end,
    file_completions
  )

  shellcmd_completions = vim.tbl_map(
    function(item) return item:gsub('([ ()%%#$])', '\\%1') end,
...
function CustomCompletion(_, cmd_line, _)
  ...
  completions = vim.tbl_map(
    function(entry) return entry:gsub('[ %[%]()%%#$]', '\\%0') end,
    completions
  )
  ...
  return vim.tbl_map(function(entry)
    if current_cmd ~= '' then return current_cmd .. ' ' .. entry end
    return entry
  end, completions)
end
        ...
        function()
          vim.cmd(
            string.format(
              ...
              title,
              (escaped_cmd .. vim.fn.shellescape('; sleep 0.05', false))
            )
          )
        end
      )
...
vim.keymap.set({ 'n', 'i', 't', 'v' }, '<M-w>', function()
  ...
  vim.fn.execute 'bd!'
  if win_id == vim.api.nvim_get_current_win() then pcall(vim.api.nvim_win_close, 0, true) end
