...
local function apply_text_edits(text_edits, bufnr, offset_encoding, dry_run, undojoin)
  ...
  then
    ...
    if #new_lines > 1 and new_lines[#new_lines] == '' then table.remove(new_lines) end
  ...
  elseif dry_run then
    ...
    if undojoin then vim.cmd.undojoin() end
...
function M.get_format_clients(options)
  ...
  if vim.lsp.get_clients then
    ...
    clients = vim.tbl_filter(
      function(client) return client.supports_method(method, { bufnr = options.bufnr }) end,
      clients
    )
  end
...
function M.format(options, callback)
  options = options or {}
  if not options.bufnr or options.bufnr == 0 then options.bufnr = vim.api.nvim_get_current_buf() end
  ...
  if options.async then
    ...
    do_format = function(idx, client)
      if not client then return callback(nil, did_edit) end
